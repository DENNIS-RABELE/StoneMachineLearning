...
from .serializers import (
    ...
    ClusterInsightSerializer, MLMetricsSerializer, BetOptionRecommendationSerializer
)
...
from .services.dashboard_visuals import build_dashboard_context

logger = logging.getLogger(__name__)


def dashboard_view(request):
    """Render a lightweight visual dashboard for ML insights."""
    time_window_days = request.GET.get('days')
...
def dashboard_data_view(request):
    """Return dashboard data as JSON for future chart integrations."""
    time_window_days = request.GET.get('days')
    top_n = request.GET.get('top')
    context = build_dashboard_context(
        time_window_days=int(time_window_days) if time_window_days else None,
    ...
    payload = {
        ...
        'cluster_rows': context['cluster_rows'],
        'top_bettors': context['top_bettors'],
        'top_characters': context['top_characters'],
        'filters': context['filters'],
        'latest_model': (
            {
                'id': context['latest_model'].id,
                'name': context['latest_model'].name,
                'n_clusters': context['latest_model'].n_clusters,
                'num_samples_trained': context['latest_model'].num_samples_trained,
                'silhouette_score': context['latest_model'].silhouette_score,
                'davies_bouldin_score': context['latest_model'].davies_bouldin_score,
                'inertia': context['latest_model'].inertia,
            }
            if context['latest_model']
            else None
        ),
        'historical_error': context['historical_error'],
    }
...
class BetOptionKnowledgeViewSet(viewsets.ReadOnlyModelViewSet):
    """Read-only access to the bet option knowledge base dataset."""

    queryset = BetOptionKnowledgeRow.objects.select_related("character", "option")
    serializer_class = BetOptionRecommendationSerializer
    permission_classes = [IsAuthenticated]
    ...
    def get_queryset(self):
        ...
        if version:
            queryset = queryset.filter(version=str(version))
        character_id = self.request.query_params.get("character_id")
        if character_id:
            queryset = queryset.filter(character_id=int(character_id))
        bet_type = self.request.query_params.get("bet_type")
        if bet_type:
            queryset = queryset.filter(option__bet_type=str(bet_type))
        return queryset

    @action(detail=False, methods=["get"])
    def recommend(self, request):
        """Recommend top option_code(s) for a character by highest p_win."""
        raw_character_id = request.query_params.get("character_id")
        ...
        try:
            ...
            return Response(
                {"error": "character_id must be an integer"},
                status=status.HTTP_400_BAD_REQUEST,
            )

        version = str(request.query_params.get("version") or "v1").strip() or "v1"
        top_n_raw = request.query_params.get("top") or "5"
        try:
            top_n = max(1, min(int(top_n_raw), 50))
        except ValueError:
            top_n = 5

        bet_type = request.query_params.get("bet_type")
        qs = (
            BetOptionKnowledgeRow.objects.filter(version=version, character_id=character_id)
            .select_related("option", "character")
            .order_by("-p_win", "option__option_code")
        )
        if bet_type:
            qs = qs.filter(option__bet_type=str(bet_type))
