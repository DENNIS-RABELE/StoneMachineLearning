...
class BettorFeatureExtractor:
    ...
    def extract_features(bettor_profile: BettorProfile) -> np.ndarray:
        ...
        features = np.array([
            ...
            bettor_profile.roi,
            min(bettor_profile.total_bets, 10000) / 10000,
        ], dtype=np.float32)
        return features
...
class KMeansBettorClusterer:
    ...
    def train(self, model_name: str, description: str = "") -> ClusteringModel:
        ...
        silhouette, davies_bouldin = self._compute_cluster_metrics(X_scaled, labels)
        inertia = self.kmeans.inertia_
        
        logger.info(
            "Model trained - Silhouette: %s, Davies-Bouldin: %s, Inertia: %.4f",
            f"{silhouette:.4f}" if silhouette is not None else "n/a",
            f"{davies_bouldin:.4f}" if davies_bouldin is not None else "n/a",
            inertia,
        )
        
        with transaction.atomic():
            existing = ClusteringModel.objects.filter(name=model_name).first()
            if existing:
                existing.bettor_clusters.all().delete()
                existing.cluster_characteristics.all().delete()
                existing.metrics.all().delete()
                clustering_model = existing
                clustering_model.description = description
                clustering_model.n_clusters = self.n_clusters
                clustering_model.random_state = self.random_state
                clustering_model.max_iterations = self.max_iterations
                clustering_model.n_init = self.n_init
                clustering_model.num_samples_trained = len(bettor_profiles)
                clustering_model.features_used = self.feature_extractor.FEATURE_NAMES
                clustering_model.inertia = float(inertia)
                clustering_model.silhouette_score = (
                    float(silhouette) if silhouette is not None else None
                )
                clustering_model.davies_bouldin_score = (
                    float(davies_bouldin) if davies_bouldin is not None else None
                )
                clustering_model.model_params = {
    ...
    def _compute_cluster_metrics(
        ...
        """Safely compute clustering metrics for small or degenerate datasets."""
        unique_labels = len(np.unique(labels))
        n_samples = len(X_scaled)

        if unique_labels < 2 or n_samples <= unique_labels:
            return None, None
