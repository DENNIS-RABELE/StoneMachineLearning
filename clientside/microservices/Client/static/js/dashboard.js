...
function updatePreloadDebugIndicator() {
  ...
  const requestedAt = info.preloadRequestedAt ? new Date(info.preloadRequestedAt).toLocaleTimeString() : "n/a";
  const loadedAt = info.preloadLoadAt ? new Date(info.preloadLoadAt).toLocaleTimeString() : "n/a";
  const readyAt = info.preloadReadyAt ? new Date(info.preloadReadyAt).toLocaleTimeString() : "n/a";
  const src = String(info.preloadRequestedSrc || info.preloadLoadSrc || info.preloadReadySrc || "").slice(0, 60);
...
function normalizeGameplaySrc(rawSrc) {
  ...
  try {
    ...
    if (url.origin === window.location.origin) {
      if (url.pathname === "/gameplay/embed" || url.pathname === "/gameplay/embed/") {
...
function buildGameplayFrameSrc(gameName) {
  ...
  const url = new URL(normalizeGameplaySrc(defaultGameplaySrc), window.location.origin);
  if (gameName) {
...
function markGameplayFrameReady(frame) {
  ...
  if (frame.id === "unityGameplayFramePreload") {
    ...
    window.__dashboardPreload.preloadReadyAt = Number(frame.dataset.preloadReadyAt);
    window.__dashboardPreload.preloadReadySrc = frame.dataset.frameSrc || frame.src || "";
    debugPreload("[Preload] preload iframe marked ready", window.__dashboardPreload);
  ...
  if (
    ...
    const { frame: pendingFrame, targetKey, targetSrc } =
      gamePreloadActivationPending;
...
function trackUnityInnerFrameReady(frame, innerFrame) {
  ...
  try {
    const loader = innerFrame.contentDocument?.getElementById(
      "unity-loading-bar",
    );
    if (!loader) {
      markGameplayFrameReady(frame);
...
function attachGameplayFrameReadyTracker(frame) {
  ...
  frame.addEventListener("load", () => {
    ...
    if (frame.id === "unityGameplayFramePreload") {
      window.__dashboardPreload = window.__dashboardPreload || {};
      window.__dashboardPreload.preloadLoadAt = Number(frame.dataset.lastLoadAt);
      window.__dashboardPreload.preloadLoadSrc = frame.dataset.lastLoadSrc;
      debugPreload("[Preload] preload iframe load event", window.__dashboardPreload);
      updatePreloadDebugIndicator();
    }
    try {
...
function setGameplayFrameSource(targetKey, targetSrc, forceReload = false) {
  ...
  if (activeFrame) {
    gamePreloadActivationPending = null;
    if (activeFrameSrc === normalizedTargetSrc && currentGameplayGame === targetKey) {
      if (forceReload) {
...
function readGameplaySchedule() {
  try {
    ...
    if (Number.isFinite(parsed.expiresAt) && parsed.expiresAt > 0 && Date.now() > parsed.expiresAt) {
      try {
...
function showActiveRoundStatsWhileGameplayRuns(targetGameplaySrc) {
  ...
  if (
    activeFrame?.dataset.frameKey === "stats-inner" &&
    inactiveFrame?.dataset.frameKey === GAMEPLAY_GAME_2 &&
    inactiveFrame?.dataset.frameSrc === normalizedGameplaySrc
  ) {
  ...
  if (
    inactiveFrame &&
    inactiveFrame.dataset.frameKey === "stats-inner" &&
    inactiveFrame.dataset.frameSrc === ACTIVE_ROUND_STATS_SRC
  ) {
    setGameplayFrameActive(inactiveFrame);
    currentGameplayGame = "stats-inner";
    persistGameplaySchedule("stats-inner", ACTIVE_ROUND_STATS_SRC);
  ...
  assignGameplayFrameSource(inactiveFrame, "stats-inner", ACTIVE_ROUND_STATS_SRC);
  setGameplayFrameActive(inactiveFrame);
  currentGameplayGame = "stats-inner";
  persistGameplaySchedule("stats-inner", ACTIVE_ROUND_STATS_SRC);
  hideGameplayCover();
...
function isPerformanceViewActive() {
  return document.getElementById("view-performance")?.classList.contains(
    "active",
  );
}
...
function beginGameplayIntermission(resumeRemaining = null) {
  ...
  const targetSrc = buildGameplayFrameSrc(GAMEPLAY_GAME_2);
  if (targetSrc) {
    console.log(`[Gameplay] Switching to preloaded game at ${roundSecondsRemaining}s`);
...
function preloadGameFrame(src) {
  ...
  window.__dashboardPreload = window.__dashboardPreload || {};
  window.__dashboardPreload.preloadRequestedAt = Number(gamePreloadIframe.dataset.preloadRequestedAt);
  window.__dashboardPreload.preloadRequestedSrc = normalizedSrc;
  window.__dashboardPreload.roundSecondsRemaining = Number(roundSecondsRemaining) || 0;
...
async function startRoundTimer() {
  ...
  roundTimerUnsubscribe = TimerManager.subscribe((timerState) => {
    ...
    if (roundSecondsRemaining <= GAME_PRELOAD_THRESHOLD_SECONDS && !gamePreloadedForRound) {
...
async function loadCharacters() {
  try {
    ...
    if (!currentRes.ok) {
      console.error("Failed to load characters:", currentRes.status, currentData);
      if (oddsBoardGrid) {
        const message =
          currentData?.error ||
          currentData?.details ||
        ...
        oddsBoardGrid.innerHTML =
          `<div class="odds-empty-board">Unable to load characters. ${message}</div>`;
      }
      return;
    }
